...
export default function Register() {
  ...
  // If user is already authenticated and lands here, sign them out and go to login
  useEffect(() => {
    if (isAuthenticated()) {
      removeToken();
      nav("/login");
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      setToast({ type: "success", text: "Account created! Redirecting…" });
  ...
  const isWeakPwd = form.password.length > 0 && form.password.length < 8;
          ...
          <div className="grid gap-1.5">
            ...
            {isWeakPwd && (
              <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded px-2 py-1">
                Tip: Use 8+ characters for a stronger password.
              </p>
            )}
          </div>

          <button
            className={`w-full rounded-lg px-4 py-2.5 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 transition ${
